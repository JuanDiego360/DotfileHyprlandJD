...
if [ "$STATUS" = "Playing" ] || [ "$STATUS" = "Paused" ]; then
    ...
    url=$($PT metadata xesam:url 2>/dev/null)

    # Fallback to YouTube, Netflix, or WhatsApp if artUrl is empty
    if [ -z "$rawUrl" ]; then
        if [ -n "$url" ] && [[ "$url" =~ youtube\.com/watch\?v=([^&/]+) ]]; then
            video_id="${BASH_REMATCH[1]}"
            rawUrl="https://img.youtube.com/vi/${video_id}/hqdefault.jpg"
        elif [ -n "$url" ] && [[ "$url" =~ youtu\.be/([^?/]+) ]]; then
            video_id="${BASH_REMATCH[1]}"
            rawUrl="https://img.youtube.com/vi/${video_id}/hqdefault.jpg"
        elif [[ "$url" == *"netflix.com"* ]] || [[ "$title" == *"Netflix"* ]]; then
